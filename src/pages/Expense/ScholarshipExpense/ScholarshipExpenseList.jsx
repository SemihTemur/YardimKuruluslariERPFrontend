...
const ScholarshipExpenseList = () => {
  ...
  const tableTitle = "Ayni Gelir Listesi";
  ...
  useEffect(() => {
    const getInKindIncomeList = async () => {
      const data = await makeRequest("get", "getInKindDonationList");
      setData(data);
      setLoading(true);
    };
    getInKindIncomeList();
    ...
    if (data) {
      const result = Object.values(
        data.reduce((acc, curr) => {
          let key = `${curr.donorFirstName} ${curr.donorLastName} ${curr.itemName} ${curr.unit}`;
