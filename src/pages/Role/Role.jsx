...
import { roleInitialValues } from "../../formik&yup/initalValues.js";
import { roleYup } from "../../formik&yup/yup.js";
...
import RoleForm from "../../components/Form/RoleForm.jsx";
...
const Role = () => {
  ...
  const {
    ...
    isPermission,
    user,
  } = useGlobalState("ROLE", "Rol", roleInitialValues, setButtonTitle);
  ...
  const getRolList = async () => {
    try {
      const response = await makeRequest("get", "getRoleList");
      setLoading(true);
  ...
  // Save
  const saveRol = async (values, { resetForm }) => {
    try {
      const response = await makeRequest("post", "saveRole", values);
  ...
  const updateRoleById = async (values) => {
    try {
      ...
      toast.success("Role başarıyla güncellendi!");
    } catch (error) {
      if (error.response && error.response.data) {
        toast.error(error.response.data.data || "Bir hata oluştu.");
      } else {
  ...
  const deleteSelectedRoles = async () => {
    try {
      ...
      toast.success("Roller başarıyla silindi!");
    } catch (error) {
      toast.error("Roller silinemedi! Lütfen tekrar deneyin.");
    }
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, "Roller Listesi.xlsx");
  };
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(14);
      doc.text("Roller Listesi", doc.internal.pageSize.width / 2, 25, {
        align: "center",
      });
      ...
      const columnStyles = {
        0: { cellWidth: 300 }, // Rol Adı
        2: { cellWidth: 300 }, // Kayıt Tarihi
        ...
        <>
          ...
          {(isUpdatedOpen || isAddOpen) && (
            <FormDialog
              ...
              onSubmit={isAddOpen ? saveRol : updateRoleById}
              initialValues={selectedData}
              validationSchema={roleYup}
              process={process}
            />
          )}
