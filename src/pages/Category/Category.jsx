...
const Category = () => {
  ...
  const getCashDonationList = async () => {
    try {
      const cashDonationData = await makeRequest("get", "getCategoryList");
      if (cashDonationData) {
  ...
  const saveCashDonation = async (values, { resetForm }) => {
    try {
      ...
      const newData = await makeRequest("get", "getCategoryList");
      setData(newData);
  ...
  const updateDonorById = async (values) => {
    console.log(values);
    try {
      await makeRequest("put", "updateCategoryById", values, selectedId);
  ...
  const deleteCashDonationById = async () => {
    try {
      await makeRequest("delete", "deleteCategoryById", null, selectedId);
  ...
  const deleteSelectedCashDonations = async () => {
    try {
      for (const row of selectedRows) {
        await makeRequest("delete", "deleteCategoryById", null, row);
      }
